...
export const useTickChartCanvas = ({
  ...
  useEffect(() => {
    ...
    const cssWidth = Math.max(
      280,
      size.width || wrapperRef.current?.clientWidth || 520,
    );
    const cssHeight = Math.max(
      280,
      size.height || wrapperRef.current?.clientHeight || 420,
    );
    ...
    ctx.fill();
  }, [canvasRef, wrapperRef, points, maxY, isCrashed, size.width, size.height]);
};
